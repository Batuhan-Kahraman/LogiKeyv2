...
namespace logikeyv2.Controllers
{
    ...
    public class IstasyonController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            List<Istasyon> liste = IstasyonManager.GetAllList(x => x.Durum == true && x.FirmaID == FirmaID);
            return View(liste);
        }
        ...
        public IActionResult Ekle(IFormCollection form)
        {
            ...
            using (var context = new Context())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        Istasyon item = new Istasyon();
                        item.Durum = true;
                        item.Adi = form["Adi"];
                        item.Aciklama = form["Aciklama"];
                        item.Kod = form["Kod"];
                        item.BolgeID = int.Parse(form["BolgeID"]);
                        item.IlID = int.Parse(form["IlID"]);
                        item.IlceID = int.Parse(form["IlceID"]);
        ...
        public IActionResult Duzenle(IFormCollection form)
        {
            ...
            using (var context = new Context())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        Istasyon item = IstasyonManager.GetByID(int.Parse(form["ID"]));
                        item.Adi = form["Adi"];
                        item.Aciklama = form["Aciklama"];
                        item.Kod = form["Kod"];
                        item.BolgeID = int.Parse(form["BolgeID"]);
