...
namespace logikeyv2.Controllers
{
    public class SurucuCikisNedeniController : Controller
    {
        ...
        {
            List<SurucuCikisNedeni> liste = SurucuCikisNedeniManager.GetAllList(x => x.Durum == true);
            return View(liste);
        }
        ...
        public IActionResult Ekle(IFormCollection form)
        {
            using (var context = new Context())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        SurucuCikisNedeni item = new SurucuCikisNedeni();
                        item.Durum = true;
                        item.Adi = form["Adi"];
                        ...
                        SurucuCikisNedeniManager.TAdd(item);
        ...
        public IActionResult Duzenle(IFormCollection form)
        {
            using (var context = new Context())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        SurucuCikisNedeni item = SurucuCikisNedeniManager.GetByID(int.Parse(form["ID"]));
                        item.Adi = form["Adi"];
                        item.FirmaID = 1;//değişçek
                        item.DuzenlemeTarihi = DateTime.Now;
                        item.DuzenleyenID = 1;//değişcek
                        SurucuCikisNedeniManager.TUpdate(item);
                        TempData["Msg"] = "İşlem başarılı.";
                        TempData["Bgcolor"] = "green";
                        return RedirectToAction("Index");
                    }
