...
namespace logikeyv2.Controllers
{
    ...
    public class YurtDisiTasimaTipiController : BaseController
    {
        ...
        public IActionResult Ekle(IFormCollection form)
        {
            ...
            using (var context = new Context())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        YurtDisiTasimaTipi item = new YurtDisiTasimaTipi();
                        item.Durum = true;
                        item.Adi = form["Adi"];
                        ...
                        YurtDisiTasimaTipiManager.TAdd(item);
        ...
        public IActionResult Duzenle(IFormCollection form)
        {
            ...
            using (var context = new Context())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        YurtDisiTasimaTipi item = YurtDisiTasimaTipiManager.GetByID(int.Parse(form["ID"]));
                        item.Adi = form["Adi"];
                        item.FirmaID = FirmaID;
                        item.DuzenlemeTarihi = DateTime.Now;
                        item.DuzenleyenID = KullaniciID;
                        YurtDisiTasimaTipiManager.TUpdate(item);
                        TempData["Msg"] = "İşlem başarılı.";
                        TempData["Bgcolor"] = "green";
                        return RedirectToAction("Index");
                    }
